...
type ServerGroupItem = { id: string; name: string; description: string; created_at: string };
...
export default function ServerGroupsList() {
  ...
  const [confirmTarget, setConfirmTarget] = useState<ServerGroupItem | null>(null);

  const load = () => {
    setLoading(true);
    api
      .get<ServerGroupItem[]>("/api/server-groups")
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) return;
    try {
      await api.post("/api/server-groups", { name: name.trim(), description: description.trim() || null });
      setName("");
      setDescription("");
      setShowForm(false);
      load();
  ...
  const requestDelete = (g: ServerGroupItem, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setConfirmTarget(g);
    setConfirmOpen(true);
  ...
  const doDelete = async (verificationToken: string) => {
    ...
    try {
      await api.delete(`/api/server-groups/${confirmTarget.id}`, {
        headers: { "X-Destructive-Verification": verificationToken },
      });
      toast("success", `Server group "${confirmTarget.name}" removed.`);
      load();
    } catch (e) {
    ...
    <div className="container app-page">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="card" style={{ marginBottom: "1.5rem", maxWidth: "28rem" }}>
          <h2 className="card-subtitle">New server group</h2>
          <div className="form-group">
            <label htmlFor="sg-name">Name</label>
            <input id="sg-name" value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Production" />
          </div>
          <div className="form-group">
            <label htmlFor="sg-desc">Description (optional)</label>
            <input id="sg-desc" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Optional" />
          </div>
              ...
              {groups.length === 0 ? (
                <tr>
                  <td colSpan={4} style={{ padding: "1.5rem", color: "var(--text-muted)" }}>
                    No server groups yet. Create one to assign servers and users.
                  </td>
                </tr>
              ) : (
                groups.map((g) => (
                  <tr key={g.id}>
                    <td>
                      <Link to={`/server-groups/${g.id}`}>{g.name}</Link>
                    </td>
                    ...
                    <td style={{ textAlign: "right" }}>
                      <Link to={`/server-groups/${g.id}`} className="btn-sm">Edit</Link>
                      {" "}
                      <button
                        type="button"
                        className="btn-sm btn-outline-danger"
                        onClick={(e) => requestDelete(g, e)}
                        title="Remove server group"
                      >
                        Remove
                      </button>
                    </td>
      ...
      <DestructiveVerificationModal
        open={confirmOpen}
        title="Remove Server Group"
        message={confirmTarget ? `Remove server group "${confirmTarget.name}"? This will remove the group and its server/user assignments. This cannot be undone.` : ""}
