...
export default defineConfig({
    ...
    server: {
        ...
        hmr: (function () {
            var host = process.env.VITE_HMR_HOST;
            var secure = process.env.VITE_HMR_SECURE === "true" || process.env.VITE_HMR_SECURE === "1";
            if (!host)
                return true;
            if (secure) {
                return { host: host, port: parseInt(process.env.VITE_HMR_PORT || "443", 10), protocol: "wss" };
            }
            return { host: host, port: 3000, protocol: "ws" };
        })(),
        proxy: {
            ...
            "/api": {
                ...
                configure: function (proxy) {
                    proxy.on("error", function (err) {
                        console.warn("[vite proxy] backend unreachable (is port 8000 running?):", err.message);
                    });
                },
            },
        },
